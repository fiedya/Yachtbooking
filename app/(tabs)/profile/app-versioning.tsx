	...
	return (
		<View style={{ flex: 1, backgroundColor: '#fff' }}>
			...
			<ScrollView contentContainerStyle={styles.container}>
				...
				<Text style={styles.currentVersion}>Aktualna wersja: {Constants.expoConfig?.version}</Text>
				{error && <Text style={styles.error}>{error}</Text>}
				{versions.map((v, idx) => (
					<View key={v.version} style={styles.card}>
						<Text style={styles.version}>v{v.version} <Text style={styles.date}>({v.date})</Text></Text>
