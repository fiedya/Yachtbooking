...
export default function AllNotesScreen() {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToUpcomingBookings(
      ...
      (error) => {
        console.error("[ALL NOTES] bookings snapshot error:", error);
        setLoading(false);
      },
    );
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToNotesForBookingIds(
      ...
      (error) => {
        console.error("[ALL NOTES] notes snapshot error:", error);
        setLoading(false);
      },
    );
  ...
  useEffect(() => {
    const unsub = subscribeToAllUsers(
      ...
      (error) => {
        console.error("[ALL NOTES] users snapshot error:", error);
      },
    );
  ...
  const filteredAndSortedNotes = useMemo(() => {
    let result: NoteRow[] = notes.map((note) => ({
      note,
    ...
    if (searchText.trim()) {
      ...
      result = result.filter(({ note, booking }) =>
  ...
  return (
    <View style={theme.screen}>
      ...
      >
        <TextInput
          placeholder="Szukaj po rezerwujacym, jachcie lub notatce..."
          value={searchText}
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
          ...
          <SortButton value="userName" label="Rezerwujacy" />
          <SortButton value="createdAt" label="Data utworzenia" />
          <SortButton value="bookingDay" label="Dzien rezerwacji" />
      ...
      <FlatList
        ...
        ListEmptyComponent={
          !loading ? (
            <Text style={theme.textMuted}>
              {searchText
                ? "Brak notatek spelniajacych kryteria"
                : "Brak notatek"}
            </Text>
          ) : null
        }
